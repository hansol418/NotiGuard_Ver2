"""
R2 스토리지를 사용하는 service.py 수정 버전
기존 service.py의 파일 업로드 부분만 변경
"""
...
from core.storage import save_file, IS_RAILWAY
...
def list_attachments(post_id: int) -> List[Dict]:
    ...
    for r in rows:
        res.append({
            ...
            "filePath": r["file_path"],  # R2 URL 또는 로컬 경로
...
def get_latest_popup_for_employee(employee_id: str) -> Optional[Dict]:
    ...
    for p in popups:
        ...
        if matches:
            ...
            # 이미지가 있으면 같이 내려줌 (R2 URL 또는 로컬 경로)
            if img:
                payload["imagePath"] = img.get("filePath")
